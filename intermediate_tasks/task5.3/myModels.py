...
class Pix2pix(nn.Module):
    ...
    def train_epoch(self, e, batch_num, mask,rectified, target):
        ...
        # target nx240x240
        inp = torch.cat((mask.to(device), rectified.to(device)),1)
        fake_images = self.generator(inp)
    ...
    def learn_D(self, inp, fake_images, target):
        ...
        fakepairs = torch.cat((inp, fake_images.detach()), 1)
        pred_fake = self.discriminator(fakepairs)
        loss_D_fake = self.criterionGAN(pred_fake, False)

        realpairs = torch.cat((inp, target), 1)
    ...
    def learn_G(self, fake_images, inp, target, mask):
        self.switch_off_discriminator()
        self.optimizer_G.zero_grad()
        fakepairs = torch.cat((inp, fake_images.detach()), 1)
        pred_fake = self.discriminator(fakepairs)
        loss_G_GAN = self.criterionGAN(pred_fake, True)
        loss_G_L1 = self.criterionL1(fake_images*mask, target*mask)*10
        loss_G_MSE = self.criterionMSE(fake_images*(1-mask), target*(1-mask))*100
        loss_G = (loss_G_L1 + loss_G_GAN + loss_G_MSE)/2
        loss_G.backward()
...
class Discriminator(nn.Module):
    ...
    def __init__(self, input_nc = 3, ndf = 64, norm_type = 'batch'):
...
class Generator(nn.Module):
    ...
    def __init__(self, input_nc = 2, output_nc = 1, ngf = 64, norm_type = 'batch', use_dropout = True):
